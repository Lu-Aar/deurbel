use esp_idf_svc::hal::{gpio::*, delay::Delay};

use core::iter::Iterator;

pub struct Gongcontrol<T: Pin> {
    address: u32,
    period: u8,
    switch_type: u8,
    delay: Delay,
...
impl<T: Pin> Gongcontrol<T> {
    pub fn new(address: u32, period: u8, switch_type: u8, pin: PinDriver<'static, T, Output>,) -> Self {
        println!{
    ...
    fn send_start_pulse(&mut self) {
        ...
        self.delay.delay_us(self.period as u32); 
        let _ = self.pin.set_low();
        self.delay.delay_us(self.period as u32 * 10 + (self.period as u32 >> 1)); 
    }
    ...
    fn send_bit(&mut self, bit: bool) {
        if bit {
            ...
            self.delay.delay_us(self.period as u32); 
            let _ = self.pin.set_low();
            self.delay.delay_us(self.period as u32 * 5); 
            let _ = self.pin.set_high();
            self.delay.delay_us(self.period as u32); 
            let _ = self.pin.set_low();
            self.delay.delay_us(self.period as u32); 
    ...
    fn send_stop_pulse(&mut self) {
        ...
        self.delay.delay_us(self.period as u32);
        let _ = self.pin.set_low();
        self.delay.delay_us(self.period as u32 * 40);
    }
}